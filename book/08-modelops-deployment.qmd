...
::: {.callout-tip}
You can read more about the tradeoffs between online real-time inference, asynchronous inference, and offline batch transform and how they can influence model serving approaches in @sec-ml-serving.
:::
...
Now let's think through how the deployment pipeline would look for the LLM we fine-tuned for the AI-powered customer support assistant project.

### Similarities and Differences Between LLM and Traditional ML Model Deployment

Deploying an LLM shares foundational concepts with traditional models. Both require versioning, a serving infrastructure, APIs, and monitoring.

The main distinction is the sheer size and resource requirements of LLMs. A traditional model like our housing price predictor might have thousands or millions of parameters. In contrast LLMs have billions of parameters and can require tens or even hundreds of gigabytes of memory just to be loaded. This leads to many downstream challenges:

- Hardware Demands: While many traditional models can run inference on standard CPUs, LLMs almost always require powerful, specialized hardware like GPUs/TPUs to provide responses in a reasonable timeframe.
- Inference Performance: LLM inference is computationally intensive. Performance is not measured by a single prediction latency but by metrics like **Time to First Token (TTFT)** and **throughput** (tokens per second).
- Cost: The combination of specialized hardware, high memory usage, and the need for always-on infrastructure makes serving LLMs significantly more expensive than serving most traditional ML models.
- Specialized Serving Frameworks: A simple FastAPI wrapper, while sufficient for many models, is often inadequate for production LLM serving. Specialized frameworks like vLLM or Text Generation Inference (TGI) are designed to optimize LLM performance through techniques like continuous batching, paged attention, and quantization, which are essential for handling concurrent users efficiently.

:::{.callout-note}
What kind of TTFT and throughput would be acceptable for serving the LLM for our AI-powered customer support assistant?
:::

### Serving options

The right serving solution depends heavily on your use case, from initial prototyping to a large-scale production system. The landscape of tools is vast and rapidly evolving.

#### Local Prototyping & Experimentation

For testing, single-user demos, or internal prototypes, simple tools are often best.

- Ollama, LM Studio: These desktop applications provide an incredibly easy way to download, run, and interact with open-source LLMs locally. They often include a built-in API server, making them perfect for initial development and testing.
- Llama.cpp: A C/C++ implementation for running models efficiently on CPUs and GPUs across various hardware, ideal for resource-constrained environments or edge deployments.
- Hugging Face Transformers with a simple API wrapper: Using a library like FastAPI to wrap a model loaded from the `transformers` library is a great way to build a custom prototype.

#### Scalable, Production Grade Deployment

When moving to production with multiple users, you need a more robust and optimized solution.

- vLLM, Text Generation Inference (TGI): These are open-source, high-performance serving engines designed specifically for LLMs.
- Cloud-Based Services: For teams that want to offload infrastructure management, major cloud providers offer powerful solutions.
  - AWS SageMaker
  - Google Cloud Vertex AI
  - Azure Machine Learning

:::{.callout-note}
Note: This space is evolving at light speed. As the LLM ecosystem matures, new ways to serve LLMs are coming up. Consider using AI tools and keep yourself up-to-date.

As always, remember that AI tools might not give you all the facts, and have no idea about your needs and constraints. AI tools might be able to guide you and give you a starting point, but the final decision and responsibility of the serving infra always lie with you, the expert.
:::


### Deciding on a Serving Solution

To choose the right serving stack for the AI-powered customer support assistant, you must ask a series of critical questions that balance performance, cost, and operational requirements.

- Model Size: How large is the fine-tuned model? This dictates the VRAM requirements and the number/type of GPUs needed.
- Context Length: Does the model need to support a large context window? This can dramatically increase memory usage during inference.
- Performance Metrics: What is the acceptable TTFT and tokens-per-second rate for a good user experience?
- Optimizations: Does the serving stack support key optimizations like quantization to reduce cost?
- Deployment Environment: Will this be deployed in the cloud, on-premises, or in a hybrid setup?
- Cost: What is the budget? Consider hardware costs, cloud service fees, and engineering maintenance effort.
- Resource Management: Can the service scale to zero or "sleep" during periods of inactivity to save costs? If so, what is the "cold start" time to wake the model up?
- Traffic Patterns: What is the expected traffic? Is it constant, or does it have predictable peaks?
- Compliance and Privacy: Does the user input contain sensitive or PII? Are there compliance requirements that mandate where data can be processed? How is data protected in transit and at rest?
- Failure: What happens if the model fails completely or partially? How is the customer request handled in such cases?


:::{.callout-note}
Can you think of other questions which would help us decide the optimal serving stack for the AI-powered customer support assistant?

Example 1: how will you handle model updates? How does the chosen stack integrate with your existing monitoring and logging tools?

Example 2: Should user queries be preprocessed before being sent to the LLM? Is any data augmentation or cleaning required? Should we implement logic to rewrite or correct malformed queries?
:::

Given your requirements and constraints, you should almost always be iterative. You should **start with a simple, "hacky" solution** to quickly build a prototype and demonstrate the model's potential to stakeholders. This enables you to gather early feedback and validate the use case with minimal investment. Once the value is proven, you can should invest the resources to build a robust, scalable, and secure production system. This iterative approach de-risks the project and ensures that engineering effort is focused on solutions that have a demonstrated business impact.

### Try Out Local LLM serving

We have talked a lot about LLMs having significantly more resource requirements during serving. Let's use a free and open-source tool called Ollama to get a hands-on feel for these requirements. This will help us get a sense of these requirements in real life.

First, install [Ollama](https://ollama.com/) on your local machine. For Linux, you can run the following command in your terminal:

```bash
curl -fsSL https://ollama.com/install.sh | sh
```

Download the software from the Ollama website for MacOS or Windows.

Ollama provides a library of open-source models that you can download and run. You can explore the available models on the [Ollama Models Library](https://ollama.com/search).

:::{.callout-note}
You could also serve the fine-tuned gpt2 model via Ollama, after you convert the model to GGUF (GGML Universal File Format).
:::

Let's try serving three models of different sizes to observe the difference in resource consumption and performance.

1. A very small model: `tinyllama` (1.1B parameters, ~638MB). This model is designed to run on most consumer machines.
```bash
ollama run tinyllama:1.1b
```

2. A medium-sized model: `llama3:8b` (8B parameters, ~4.7GB). This is a popular and powerful model that requires more memory.
```bash
ollama run llama3:8b
```

3. A large model: `mixtral:8x22b` (A 141B parameter Sparse Mixture-of-Experts model, ~80GB). This model requires significant VRAM and is unlikely to run smoothly on standard consumer hardware.
```bash
ollama run mixtral:8x22b
```

Can your local computer serve all of these models? As you interact with each one, what kind of TTFT and throughput do you notice? Given what you know about your computer's hardware, can you estimate what kind of hardware would be needed to serve the fine-tuned LLM in production? What if you needed to serve multiple instances of that model in parallel to support thousands of customers?

Hypothetically, if you could run N number of machines like your personal computer in parallel, how many machines do you think would be required to support the AI-powered customer support assistant? This exercise should give you a tangible sense of the scaling challenges involved in LLM deployment.

### Beyond Model Deployment: Building Complete Solutions

While this chapter focuses extensively on model deployment: the process of making a trained model available for inference in production, it is crucial to understand that model deployment is just one component of solution deployment. The model itself is rarely the complete solution, rather it is a critical piece within a broader system that addresses business requirements.

Model deployment refers to making the model accessible within the production environment. The model deployment ensures that the model can reliably generate predictions when called upon. Solution deployment on the other hand, encompasses the entire end-to-end system that delivers business value.

So, for the AI-powered customer support assistant, model deployment involves serving the fine-tuned language model. But the complete solution includes user authentication, conversation history management, integration with knowledge bases, rephrasing/standardization of user queries, escalation pathways to human agents, and compliance with data privacy regulations.

The key takeaway is that your model deployment strategy must align with the broader solution architecture. Choice of serving architecture, libraries, and API design are all influenced by how the model fits into the complete business workflow. This reinforces the importance of understanding the business problem thoroughly (as emphasized in earlier chapters) and considering the full ML system design principles.

Additionally, as AI tools for tech workers improve, understanding new concepts and learning becomes easier, and the boundaries between tech job titles like data science and software engineering blur. Successful ML practitioners must think beyond model performance metrics to consider how their models integrate into solutions that create meaningful business impact. For example, an LLM which can answer most user queries with super high accuracy is meaningless if it takes 30 seconds to generate the first tokens.

The most impactful ML practitioners understand that deploying models effectively requires collaboration across teams, consideration of user experience, and alignment with business operations—skills that extend far beyond traditional data science boundaries.
