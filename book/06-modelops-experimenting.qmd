...
::: {.callout-note}
Codes below were run on a MacOS computer with an M3 Pro chipset and MPS (Metal Performance Shaders) support. If you are using a different operating system or hardware, you may need to adjust some settings or dependencies to ensure compatibility.
:::
...
- Model Choice (gpt2): We will use gpt2 for this exercise because it is a well understood foundational model. At 124 million parameters, it is also small enough to be fine-tuned on consumer hardware. The principles learnt while fine-tuning gpt2 are directly applicable to much larger models.
...
The code for this exercise is built upon [LLM fine-tuning repository](https://github.com/yash91sharma/llm_finetuning). Refer to this repository for more variations and complete code.
...
::: {.callout-note collapse="true"}
Create dummy data, if you have not done so in the past chapters.

Generate a sample dataset containing 100-150 examples, and save it in the llm_training_data.jsonl file, which will be used for model fine-tuning. Ensure the dataset includes a wide range of languages, vocabulary, topics, and perspectives to maximize diversity and representativeness.
...
To further enhance your LLM fine-tuning workflow, refactor your code to use a configuration file for orchestrating experiments. This approach makes it much easier to adjust hyperparameters, model settings, and data paths without modifying your training script. Refer to the [LLM fine-tuning repository](https://github.com/yash91sharma/llm_finetuning) for an example. Adopting a config-driven workflow not only streamlines experimentation but also demonstrates advanced engineering skills, an excellent addition to your professional portfolio.
