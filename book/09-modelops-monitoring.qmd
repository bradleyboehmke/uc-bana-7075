...
In previous chapters, we discussed an AI-powered customer support assistant for a robotics company. Now, let's design a comprehensive monitoring strategy for the deployment of the LLM used for the customer support assistant.
...
This focuses on monitoring the technical performance of the system. Track metrics such as response latency, token generation speed, memory consumption, and API uptime to ensure the system consistently delivers acceptable performance, even under heavy load.

#### Data Drift Detection

Traditional drift detection methods—such as monitoring mean, variance, or outlier rates—are not directly applicable to text data. Instead, we must extract higher-level metrics to assess drift in language-based inputs. Drift in LLM systems can manifest in various forms, including changes in vocabulary, query complexity, or topic distribution. To effectively monitor these shifts, techniques like text embeddings can be employed to track changes in the semantic distribution of user queries over time, enabling detection of subtle shifts in user language and intent.
...
Concept drift occurs when the relationship between user queries and expected responses changes. For our AI-powered customer support assistant, this might manifest as:
...
- Seasonal or Contextual Shifts: Holiday seasons, new product launches, or external events (like weather/seasons in case robot performs outdoor tasks) might change the types of support queries and required responses.

**Example scenarios that would indicate concept drift:**

- Users asking "Why won't my robot charge?" might initially be answered with basic troubleshooting steps, but after a product recall due to a battery issue, the correct response should direct users to contact support immediately.
...
To address these challenges, a variety of open-source and commercial solutions are available. Tools like Evidently AI can assist with drift and performance tracking.
:::
...
- Content Safety Metrics: Monitor for inappropriate responses, toxic language, or responses that might violate company policies or safety guidelines. Libraries such as Google Perspective API enable automated detection of toxic or inappropriate responses.

::: {.callout-note}
Remember that you fine-tuned an already pre-trained LLM model. Even though you may have carefully curated your FAQs and product manuals to remove bias, the underlying LLM still carries potential biases from its original training data. These inherited biases could cause the AI-powered customer support assistant to generate unexpected or problematic responses that don't align with the company's values or customer service standards.
...
Scheduled Retraining (for example quarterly or yearly):
...
- Advantages: More responsive to actual performance issues, can address critical problems immediately.
...
**Recommended Hybrid Approach**: Implement adaptive triggers for critical issues (safety, major factual errors) while maintaining a regular schedule for general knowledge updates and performance optimization.

### Balancing Business Continuity with Model Improvement

Remember that our original problem was the robotics company's inability to scale customer support with increasing user queries. Any monitoring and retraining strategy must keep in mind that original goal.
