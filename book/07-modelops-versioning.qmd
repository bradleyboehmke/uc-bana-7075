...
In this exercise, we'll explore how model versioning and reproducibility apply to our AI-powered customer support assistant. Unlike traditional ML models, LLMs present unique challenges and considerations for versioning and reproducibility due to their size, training methodology, and stochastic nature.

### Versioning Strategy

#### Semantic Versioning for LLM

How would you adapt semantic versioning (MAJOR.MINOR.PATCH) for LLMs?

- MAJOR: Big changes like switching to a newer base model architecture or significant capability upgrades
- MINOR: Adding new features, topic domains, or notable performance improvements
- PATCH: Small fixes, prompt tweaks, or adding a few more training examples

#### Naming Conventions

What naming patterns would make it easy for everyone on your team to understand what they're working with?

- Example: `customer-support-llama3-8b-full-v1.2.0` vs. `customer-support-llama3-8b-lora-v1.2.0`
- What information should always be in your model names? Base model? Size? Fine-tuning approach?
- How could you distinguish between models trained for different product lines or customer segments?

#### Other LLM Version Attributes

Beyond standard model versioning attributes, what LLM-specific metadata should be tracked?

- Base model identity and version
- Quantization details (4-bit, 8-bit, etc.)
- Fine-tuning approach (full, LoRA, QLoRA, etc.)
- Prompt templates and few-shot examples
- Context window size
- Temperature settings used during evaluation

### Reproducibility

Reproducibility with LLMs presents unique challenges compared to deterministic ML models due to their inherently stochastic nature. Let's explore these challenges through a practical example.

#### Example: Robot Object Recognition Problem

Consider this complex user question: `My robot sometimes identifies my flowerpot as my coffee mug because they are the same color and size, what do I do?`

This is multi-layered and complicated question. To answer well, the AI-powered customer support assistant needs to:

- Understand that this is inherently an object recognition issue which the user is facing
- Identify potential causes (similar visual features)
- Use it's knowledge (fine-tuning) to find the correct solution for the user
- Suggest step by step solution

:::{.callout-note}
Did we miss something from the above steps, which needs to be included to give a complete, robust, and useful answer to the user? 

How about user/product metadata? Version of hardware/software user might be using?
:::

Now think about users from different background, technical expertise, language barriers etc. Users could ask the same question in many different forms, some examples:

- My robot's object recognition system is exhibiting classification errors between two distinct objects: a flowerpot and a coffee mug. The primary confounding variables appear to be identical color profiles and similar dimensions. What calibration or retraining protocols can be implemented to resolve this ambiguity?
- Ugh, my robot's tripping. It keeps mistaking my succulent's pot for my coffee mug. How do I fix this glitch?
- My robot. It see flowerpot, think coffee mug. Big problem.
- It keeps bringing me a plant when I want a drink. Help!
- It's so frustrating! Every other day, my robot tries to water my coffee mug.

As you can see from these examples, user queries can range from highly technical and precise to extremely casual and vague. This variety presents a significant challenge for LLM systems, which must understand the core problem regardless of how it's expressed. With the above knowledge, how would you define `reproducibility` when it comes to LLMs?

Two versions of our fine-tuned LLMs might produce notably different answers for the above user question. How can we systematically compare and evaluate them? Some strategies could be:

- Human evaluation: Systematic assessment using human raters with rubrics for coherence, accuracy, and helpfulness
- Automated evals: Benchmark suites like HELM or custom test sets measuring performance on specific tasks
- Using LLMs as evaluators: Leveraging more powerful LLMs to evaluate responses from fine-tuned models
- User metrics: Tracking engagement, resolution rates, and satisfaction scores in production
