...
In this exercise, we will apply DataOps principles to design and implement a data pipeline for our AI-powered customer support assistant. The goal is to ingest data from various sources (tickets, FAQs, manuals), process it, and create a unified, versioned dataset suitable for fine-tuning an LLM.

A key difference from traditional ML data pipelines is the sheer volume of data often required for fine-tuning Large Language Models. While traditional models might train on thousands or millions of structured records, LLMs benefit from vast quantities of diverse, often unstructured, text data. This emphasis on volume and variety means that scalability, automation, and efficient processing are not just best practices but necessities for building a successful LLM data pipeline.

The focus is on collecting, processing, and formatting real-world support data (tickets, call logs, user manuals, FAQs) into a structure suitable for LLM training.

### 1. Pipeline Design and Configuration

A production-grade pipeline should be modular, configurable, and resilient. We will break our pipeline into distinct, reusable functions and manage all configurations externally.

First, let's define our data sources and pipeline settings in a configuration file. This practice prevents hardcoding paths and parameters, making the pipeline adaptable to different environments (development, staging, production).

````yaml
# filepath: /path/to/config/llm_data_pipeline.yaml
# Configuration for the LLM data preparation pipeline

# Input data sources
sources:
  tickets: 'data/raw/support_tickets.csv'
  faqs: 'data/raw/faqs.json'
  manuals: 'data/raw/manuals.csv'

# Output settings
output:
  path: 'data/processed/llm_training_data.jsonl'
  dvc_enabled: true

# Data processing parameters
processing:
  deduplicate_on: ['input', 'output']
  anonymize_fields: ['customer_name', 'email']

# Augmentation settings
augmentation:
  enabled: true
  model: 'gpt-4-turbo' # Use a fast and cost-effective model
  api_key_env: 'OPENAI_API_KEY' # Load API key from environment variable
````

### 2. Building a Modular and Scalable Pipeline


The raw data is assumed to be available in following formats.

```python
# customer support tickets
tickets = pd.DataFrame([
    {"ticket_id": 1, "customer_name": "Alice Smith", "email": "alice@example.com",
     "issue": "Robot thinks my window is a door, tries to walk out and falls into the yard",
     "resolution": "Sent link for user to update their home map in the phone app."},
    {"ticket_id": 2, "customer_name": "Bob Jones", "email": "bob@example.com",
     "issue": "Billing error on last invoice",
     "resolution": "Issued refund."},
    {"ticket_id": 3, "customer_name": "Alice Smith", "email": "alice@example.com",
     "issue": "Robot does not restart after OS update",
     "resolution": "Suggested hard reset..."}
])

# call transcripts
calls = pd.DataFrame([
    {"call_id": 101, "agent": "Eve",
     "transcript": "Hello, this is Eve from support. How can I help you? ...",
     "summary": "Explained subscription options."},
    {"call_id": 102, "agent": "Dan",
     "transcript": "Hi, my robot is lost trying to drive itself back from grocery store...",
     "summary": "Walked through robot location reset procedure."}
])

# user manual
manuals = pd.DataFrame([
    {"section": "Resetting your robot's location configs", "content": "To reset your robot's location settings, go to the settings page on the app or website, open 'settings', then 'location config' ..."},
    {"section": "Hard reset a robot", "content": "Your robot needs to be atleast 50% charged for a hard reset. First start by..."}
])

# FAQs
faqs = pd.DataFrame([
    {"question": "How do I reset my app's password?", "answer": "Click on 'Forgot Password' at login."},
    {"question": "How can I change my robot's name?", "answer": "Go to 'settings' in your mobile app, then..."}
])
```

Generate a synthetic dataset of 100–150 example data points using the templates above. For this exercise, these examples will serve as our training data. In a real-world scenario, your dataset should be more comprehensive, and of higher quality to ensure robust model performance.

::: {.callout-note collapse="true"}
Note: The data for LLM fine-tuning is expecting in this format:
```json
[
...
This is the format selected for the LLM exercise throughout this book. But you can chose the data format which suits your needs and constraints.
::::

We will now implement the processsing pipeline in Python. Notice how each step (loading, transforming, saving) is a separate function. This modularity makes the code easier to test, debug, and maintain. We will also include logging and error handling to make the pipeline more robust.

````python
# filepath: /path/to/code/prepare_llm_data.py
import pandas as pd
import yaml
import json
import logging
...
def load_config(config_path='/path/to/config/llm_data_pipeline.yaml'):
...
# --- 2. Data Ingestion (Modular Loaders) ---
def load_data(sources: dict) -> dict:
    """Loads data from multiple sources defined in the config."""
    dataframes = {}
    for name, path in sources.items():
        try:
            ...
            if path.endswith('.csv'):
                ...
                logging.warning(f"Unsupported file format for {path}. Skipping.")
        except FileNotFoundError:
            logging.error(f"Data file not found: {path}. Skipping.")
    return dataframes

# --- 3. Data Transformation ---
def transform_to_instruction_format(dataframes: dict) -> list:
    """Transforms various dataframes into a unified instruction-following format."""
    all_examples = []
    
    # Transform support tickets
    if 'tickets' in dataframes:
        for _, row in dataframes['tickets'].iterrows():
            all_examples.append({
                "instruction": "Resolve the following customer support ticket.",
                "input": row['issue'],
                "output": row['resolution']
            })

    # Transform FAQs
    if 'faqs' in dataframes:
        for _, row in dataframes['faqs'].iterrows():
            all_examples.append({
                "instruction": "Provide an answer to the following frequently asked question.",
                "input": row['question'],
                "output": row['answer']
            })

    # Add other data source transformations...
    
    logging.info(f"Transformed {len(all_examples)} examples into instruction format.")
    return all_examples

# --- 4. Data Processing (Cleaning & Anonymization) ---
def process_data(examples: list, config: dict) -> pd.DataFrame:
    """Cleans, anonymizes, and deduplicates the dataset."""
    df = pd.DataFrame(examples)
    
    # Anonymization (if needed, though not present in final format)
    # This step would be applied before transformation if PII was in the final output.

    # Deduplication
    initial_rows = len(df)
    df.drop_duplicates(subset=config['processing']['deduplicate_on'], inplace=True)
...
def augment_data(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    """Augments data by paraphrasing inputs using an LLM."""
    if not config['augmentation']['enabled']:
        logging.info("Augmentation is disabled.")
        return df

    api_key = os.getenv(config['augmentation']['api_key_env'])
    if not api_key:
        logging.warning("OpenAI API key not found. Skipping augmentation.")
        return df

    client = OpenAI(api_key=api_key)
    
    # For demonstration, we augment a small sample. In production, you might
    # use more sophisticated sampling or parallel processing.
    augmented_examples = []
    ...
    for _, row in sample_df.iterrows():
        try:
            response = client.chat.completions.create(
                model=config['augmentation']['model'],
                messages=[
                    {"role": "system", "content": "You are a helpful assistant. Paraphrase the following user input."},
                    {"role": "user", "content": row['input']}
                ]
            )
            paraphrased_input = response.choices[0].message.content.strip()
            ...
            logging.error(f"Failed to augment data for input: {row['input']}. Error: {e}")

    if augmented_examples:
        df = pd.concat([df, pd.DataFrame(augmented_examples)], ignore_index=True)
        logging.info(f"Added {len(augmented_examples)} augmented examples.")
    
    return df

# --- 6. Data Storage and Versioning ---
def save_data(df: pd.DataFrame, config: dict):
    """Saves the final dataset as JSONL and handles DVC tracking."""
    ...
    if config['output']['dvc_enabled']:
        logging.info("Tracking data with DVC...")
        os.system(f'dvc add {output_path}')
        logging.info(f"Run 'git add {output_path}.dvc' and commit to version your data.")

# --- Main Pipeline Execution ---
def main():
    """Main function to run the data preparation pipeline."""
    config = load_config()
    raw_dataframes = load_data(config['sources'])
    instruction_examples = transform_to_instruction_format(raw_dataframes)
    processed_df = process_data(instruction_examples, config)
    augmented_df = augment_data(processed_df, config)
    save_data(augmented_df, config)
    logging.info("LLM data preparation pipeline finished successfully.")
...
Above, we demonstrate how an existing LLM can be leveraged to paraphrase and augment the training data. While this approach can increase data diversity and improve model robustness, it should be applied judiciously to avoid introducing bias or redundancy. In practice, data augmentation strategies are often more sophisticated, incorporating advanced business logic or human-in-the-loop review to ensure quality and relevance. The following example illustrates a simple, augmentation technique as a starting point.

::: {.callout-warning}
Why might the "augmented data" generated by an LLM introduce bias, even if our original dataset was unbiased? How would the LLM's own training data, prompt phrasing, or model limitations could subtly shape outputs in ways that reflect hidden assumptions or stereotypes? What creative strategies could you use to detect, mitigate, or even leverage these biases to improve your pipeline? This is a fascinating challenge at the intersection of data quality, ethics, and AI.
:::

#### Efficiency Improvements
Identify edge cases and dependencies in the above workflow that would prevent it from scaling effectively when run on a cluster of machines rather than a single machine. For example:

- If you need to process 50GB of data, what changes would you make to the code to handle this volume within a reasonable timeframe? What kind of hardware and software would you need? 
- How will you handle failures? How about all clusters failing vs few clusters failing? What would be the retry strategy?

Note: While official figures are not published, it is estimated that base versions of advanced LLMs such as GPT-3.5 were trained on approximately 200–400GB of text data.

#### Beyond Basic Data Processing
While the current pipeline focuses on basic data processing and format conversion, it lacks essential quality assurance steps such as automated validation checks, outlier detection, and human evaluation—all of which are critical for ensuring high-quality data as emphasized throughout this book.
- Based on what we have learnt until now, can you suggest few improvements that address the above concern?
- How do we ensure that the data we get at the end is of the best quality?
- How much additional computational and human resources would be required?
- How will you decide if it is worth spending that additional resourc for higher quality, at this early stage of the project?

### Reflection: Complexity, Scalability, and Productization

Building a robust LLM data pipeline involves many decisions:

- **Data diversity**: Real-world data is messy and comes in many formats. Each source needs custom cleaning and transformation.
- **Privacy**: Ensuring anonymization is critical for regulatory compliance and maintaining user trust. Exposing sensitive data through an LLM can have serious consequences.
- **Augmentation**: Synthesizing new examples (e.g., Q&A from manuals) can boost model performance but must be done thoughtfully to avoid introducing bias or noise.
- **Modularity and Testability**: Each function can be tested independently. For example, you could write a unit test for `transform_to_instruction_format` with mock data to ensure it behaves as expected.
- **Configuration-Driven**: By changing the `config.yaml` file, you can easily switch data sources, change model parameters, or point to a different output location without touching the pipeline code.
- **Robustness**: Basic logging and error handling are included. A production system would expand on this with more detailed logging, alerts for failures, and retry mechanisms for transient errors.
- **Scalability**: While this example still uses pandas and processes data in-memory, its modular structure makes it easier to scale. For a very large dataset, you could replace the pandas-based functions in `process_data` and `augment_data` with equivalents that use **Apache Spark** or **Dask**. These frameworks would distribute the computation across multiple machines, allowing the pipeline to handle terabytes of data. The overall pipeline orchestration would remain the same.

As a challenge, try adapting this pipeline to use a distributed framework like Spark or Dask to gain hands-on experience with scaling data processing beyond the limits of a single machine.
