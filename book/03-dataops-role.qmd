...
```{mermaid}
%%| fig-cap: "DataOps includes efficient and reliable data ingestion, processing, validation, and versioning/lineage workflows to ensure that data is as high-quality as possible for the rest of the ML system."
%%| label: fig-devops
...
```{mermaid}
%%| fig-cap: "Batch processing collects data in chunks or intervals, such as on a daily cadence.  In this example, every day a batch process is ran where data is ingested from its relevant data sources, processed and fed to downstream processes in the ML system."
%%| label: fig-batch
...
```{mermaid}
%%| fig-cap: "Streaming, or real-time, systems process each observation as it comes in and then feeds that data to downstream ML systems and, simoultaneously, to a database for longterm storage."
%%| label: fig-streaming
...
> *”Poor data quality is Enemy #1 to the widespread, profitable use of machine learning, and for this reason, the growth of machine learning increases the importance of data cleansing and preparation. The quality demands of machine learning are steep, and bad data can backfire twice -- first when training predictive models and second in the new data used by that model to inform future decisions.”*^[tdwi blog: https://tdwi.org/articles/2019/04/16/diq-all-data-quality-problems-will-haunt-your-analytics-future.aspx]
...
The samples selected by nonprobability criteria are not representative of the real-world data and therefore are riddled with selection biases.[@heckman2013sample] Because of these biases, you might think that it’s a bad idea to select data to train ML models using this family of sampling methods. You’re right. Unfortunately, in many cases, the selection of data for ML models is still driven by convenience.
...
```{mermaid}
%%| fig-cap: "Feature engineering serves to bridge the gap between raw data and a machine learning model’s input requirements."
%%| label: fig-feat-eng
...
- **Feature Engineering for Machine Learning: Principles and Techniques for Data Scientists** [@zheng2018feature]: This book provides a comprehensive overview of feature engineering techniques across different data types, with practical advice and examples.
- **Feature Engineering and Selection: A Practical Approach for Predictive Models** [@kuhn2019feature]: This resource delves into advanced feature selection methods and preprocessing techniques, with a focus on model performance improvement.
- **Hands-On Machine Learning with Scikit-Learn, Keras, and TensorFlow** [@geron2022hands]: Chapter 2 of this book covers essential data preprocessing and feature engineering steps with Python code examples.
- **Online Tutorials and Blogs**:
  - [Kaggle’s Feature Engineering Course](https://www.kaggle.com/learn/feature-engineering): A free course offering hands-on practice with feature engineering in real-world datasets.
  - [Towards Data Science Feature Engineering Articles](https://towardsdatascience.com): A collection of articles covering practical and innovative feature engineering techniques.
- **Papers with Code**:
  - [Feature Engineering Papers](https://paperswithcode.com/task/feature-engineering): A repository linking research papers on feature engineering to practical code implementations.
...
1. **Decreased Model Generalizability**: Models trained with leaked data perform exceptionally well during testing but fail to generalize to unseen production data, undermining their real-world utility. In MLOps, this can result in costly failures when the system is deployed.
...
4. **Uniqueness**: Identifying and removing duplicate records to prevent data redundancy and biased model training. For instance, deduplicating customer profiles ensures accurate customer segmentation.
...
![Incorporate Validation at Multiple Stages](images/ml_workflow_validation.png)
:::
...
```{mermaid}
%%| fig-cap: "Data versioning tracks historical versions of a given dataset. This makes it explicit as to which instance of the dataset was used for a given model (i.e. v1.1.0) and also allows rolling back to previous versions (i.e. v1.0.2) if necessary."
...
```{mermaid}
%%| fig-cap: "Data lineage tracks the movement and transformations of data from its original source(s) to its destination, including how the data was changed and why."
...
Let’s continue with the AI-powered customer support assistant scenario. Assume that, to meet business needs, you will need to fine-tune a base large language model (LLM) using company-specific data. Drawing on the DataOps principles from this chapter, consider the following questions to refine your approach.

### Data Source and Processing
- What have you learned from initial data exploration that will inform your fine-tuning strategy?
- What types of data (e.g., chat logs, support tickets, user manuals, FAQs) are most relevant for fine-tuning the LLM?
- Do you have sufficient data volume and diversity to support robust fine-tuning? If not, how might you address gaps?
- Is the data available in a consistent and usable format, or will additional cleaning and preprocessing be required?
- How will you assess and ensure data quality (e.g., accuracy, completeness, consistency)?
- Are there language considerations (e.g., multilingual support, need for translation)? At what stage in the pipeline should translation or language normalization occur?
- If data is limited, what strategies could you use to augment or generate additional training examples?

### Data Versioning and Lineage
- How will you track versions of your training data, including updates to user manuals, FAQs, and support logs?
- What tools or processes will you use to document data lineage, ensuring you can trace the origin and transformation of all data used for fine-tuning?
- How will you handle updates to the underlying product (e.g., new robot versions) that may render some data obsolete or require new data to be incorporated?
- What is your plan for removing outdated or irrelevant information from the fine-tuned LLM?

### Data Validation and Quality Assurance
- What validation checks will you implement to ensure that only high-quality, relevant data is used for fine-tuning?
- How will you detect and handle anomalies, duplicates, or inconsistencies in the data?
- At which stages of the pipeline (ingestion, preprocessing, prior to training) will you perform these checks?

### Iterative Evolution and Feedback
- How frequently do you expect new data to become available, and how will you incorporate it into the fine-tuning process?
- What mechanisms will you use to monitor model performance and data quality over time?
- How will you manage the iterative nature of LLM fine-tuning, including retraining schedules, evaluation, and rollback if issues are detected?
- How will you balance the need for rapid updates with the risk of introducing errors or data drift?

### Context and Personalization
- Does your solution require additional context (e.g., user behavior, expertise level, robot version) to provide accurate and relevant responses?
- Should this context be incorporated during model fine-tuning, or provided dynamically at inference time via prompts?
- How will you ensure that the assistant remains up-to-date and aligned with evolving user needs and product changes?

Reflect on these questions and document your answers, using both the information provided and your own reasoning. This exercise will help you apply DataOps best practices to the unique challenges of fine-tuning and maintaining LLMs in a real-world, production environment.
